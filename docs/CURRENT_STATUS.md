...
| 홈스테이 신청 | 카카오 주소검색, 구역반 자동 매핑, 수동 수정, 개인정보 동의, 전체·침대 수용 인원 분리, 접수 조회/수정/취소 |
| 홈스테이 등록 기준 | 수용 인원 최소 2명, 이름·전화·주소 기반 중복 차단, 권한 관리자 영구 삭제 |
| 자원봉사자 신청 | 원페이지 신청, 복수 지원 분야, 복수 언어, 주소 기반 구역반 자동 매핑/수동 설정 및 인적 사항 구역 연동, 자동 신청일/서명 |
| 봉사 일정 | 운영자 일정 등록·수정·삭제, 정원·중복 시간 검증, 봉사자 신청·취소 |
...
| 대시보드 | 수용 인원별 가정 수, 남성 지정·여성 지정·성별 무관 수용 가능 인원, 구역별 신청 가정·수용 인원·목표 달성률, 침대방, 반려동물, 성별, 호스트 연령대 분포 |
| 운영자 계정 | 회원가입, 비밀번호 변경, super admin 승인/권한 관리 |
| 개인정보 접근 | `admin`, `privacy_admin`, `super_admin` 역할 분리 |
| 엑셀 다운로드 | 홈스테이 및 자원봉사자 관리자 다운로드 지원 |
| 종이 신청 | 개인정보 관리자가 등록 후 임시 PIN 발급, 기존 신청 PIN 재발급, 접수 경로 및 개인정보 동의 필터 지원 |

신청자가 직접 수정·취소할 수 있는 상태는 `submitted`입니다. `confirmed` 또는 `canceled` 상태에서는 직접 변경을 잠그고 성당 문의를 안내합니다. 운영자 신청 현황에는 최근 7일간 신청자 수정 건수를 표시합니다.
