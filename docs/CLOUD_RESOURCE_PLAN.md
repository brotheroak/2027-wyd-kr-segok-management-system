...
## DB 사이즈 및 RDS/Cloud SQL 필요 여부

결론: 현재처럼 Google Cloud Run에 이미 배포 중인 운영 구조에서는 외부 관리형 DB가 필요하며, GCP에서는 Cloud SQL PostgreSQL이 그 역할을 합니다. AWS로 이전할 때의 동등한 선택지는 RDS PostgreSQL입니다.

단일 VM에 애플리케이션 1개만 올리는 별도 운영안이라면 최대 동시접속 100명, 첨부파일 없음, 신청 수 수천 건 기준 SQLite도 충분합니다. 다만 이 판단은 현재 Cloud Run 운영에는 적용하지 않습니다.
...
- Cloud Run처럼 컨테이너 파일 시스템이 영구 저장소가 아닌 구조를 쓴다.
...
따라서 이 프로젝트의 현재 100명 동시접속 기준 권장안은 `Cloud Run + Cloud SQL PostgreSQL + Cloud Run max-instances 상한 + 앱 내부 웹퍼널`입니다. `SQLite 유지 + 단일 VM + 촘촘한 백업`은 별도의 저비용 대안일 뿐, 현재 운영 배포의 기준안은 아닙니다.
