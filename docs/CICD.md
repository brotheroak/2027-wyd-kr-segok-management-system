# Argo CD와 GitHub Actions를 활용한 GitOps CI/CD 구축 가이드

이 문서에서는 본 프로젝트의 지속적 통합(CI) 및 지속적 배포(CD) 파이프라인의 설계와 운영 방법을 설명합니다. 본 프로젝트는 선언적인 인프라 정의와 자동화를 위해 **GitOps** 패턴을 채택하고 있으며, CI 도구로 **GitHub Actions**, CD 도구로 **Argo CD**를 사용합니다.
...
## 5. 시나리오 B: Google Cloud Run 자동 CI/CD 가이드 (GitHub Actions)
