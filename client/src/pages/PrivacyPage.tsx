      ...
      <article className="panel policy-panel">
        ...
        <PolicySection title="2. 수집하는 개인정보">
          <ul>
            ...
            <li>순례자 출석·위치 정보: 바코드 확인 시각, 운영 체크 지점, 기기 위치 좌표, GPS 정확도, 체크 지점과의 거리</li>
            <li>Q&amp;A 정보: 작성자명, 질문 내용, 접수 및 답변 시각, 문의 비밀번호의 해시값</li>
          </ul>
        </PolicySection>

        <PolicySection title="3. 이용 목적과 법적 근거">
          <p>개인정보는 홈스테이 신청 확인, 신청서 수정 및 취소, 호스트 연락과 사전 교육 안내, 운영자 검토, 순례자 요청 조건과의 매칭, 순례자 카드 발급·발송, 배정 호스트의 식단 확인, 행사 출석 확인과 안전한 인원·동선 관리, 문의 응대, 시스템 보안과 품질 개선을 위해 이용됩니다. 홈스테이 신청 시 개인정보 수집·이용 동의를 별도로 확인하며, 종이 신청은 개인정보 관리자가 입력한 뒤 신청자에게 임시 조회번호를 안내할 수 있습니다. 호스트 로그인은 성명, 연락처, 4자리 비밀번호로 진행하고 순례자 카드는 개인별 만료 접속 코드로 엽니다. 비밀번호 원문은 저장하지 않으며 접속 코드는 암호화 값과 조회용 해시로 관리합니다.</p>
        </PolicySection>
        ...
        <PolicySection title="8. 안전성 확보 조치">
          <p>본 시스템은 운영자 권한을 일반 운영자와 개인정보 관리자로 분리합니다. 일반 운영자에게는 이름, 연락처, 이메일, 주소 등 주요 개인정보가 익명화되어 표시되며, 원본 개인정보와 식단·알레르기·발열 등 건강 관련 민감정보 및 출석·위치 이력은 개인정보 관리자만 열람할 수 있습니다. 출석 확인은 지정된 체크 지점의 허용 반경과 기기 GPS 좌표를 서버에서 대조하며, 기기 원시 좌표는 암호화 저장합니다. 순례자 카드는 예측하기 어려운 전용 링크로 제공되며 링크 비밀값은 암호화 저장됩니다. 확정된 호스트는 접수 인증 후 자기 가정에 배정된 순례자의 등록 정보와 식단·알레르기 정보만 열람할 수 있습니다. 링크나 바코드를 타인에게 전달하지 않아야 합니다.</p>
        </PolicySection>
