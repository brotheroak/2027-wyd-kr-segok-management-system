...
async function setupPg() {
  ...
  try {
    ...
    await client.query(`
      ...
      CREATE TABLE IF NOT EXISTS pilgrims (id TEXT PRIMARY KEY, pilgrim_no TEXT UNIQUE NOT NULL, name TEXT NOT NULL, baptismal_name TEXT NOT NULL DEFAULT '', email TEXT NOT NULL DEFAULT '', preferred_language TEXT NOT NULL DEFAULT 'en', access_token TEXT, access_token_hash TEXT UNIQUE, access_token_expires_at TEXT, gender TEXT NOT NULL, diocese TEXT NOT NULL, region TEXT NOT NULL, grade TEXT NOT NULL, age INTEGER NOT NULL, diet_type TEXT NOT NULL DEFAULT '일반식', diet_notes TEXT NOT NULL DEFAULT '', allergies TEXT NOT NULL DEFAULT '', health_notes TEXT NOT NULL DEFAULT '', fever_status TEXT NOT NULL DEFAULT '정상', host_application_id TEXT REFERENCES homestay_applications(id) ON DELETE SET NULL, created_at TEXT NOT NULL, updated_at TEXT NOT NULL);
      ...
      CREATE TABLE IF NOT EXISTS attendance_checkpoints (id TEXT PRIMARY KEY, name TEXT NOT NULL, postcode TEXT NOT NULL DEFAULT '', address TEXT NOT NULL, address_detail TEXT NOT NULL DEFAULT '', latitude DOUBLE PRECISION NOT NULL, longitude DOUBLE PRECISION NOT NULL, radius_m INTEGER NOT NULL DEFAULT 100, active BOOLEAN NOT NULL DEFAULT TRUE, created_by TEXT NOT NULL, created_at TEXT NOT NULL, updated_at TEXT NOT NULL);
      CREATE TABLE IF NOT EXISTS pilgrim_attendance_logs (id TEXT PRIMARY KEY, pilgrim_id TEXT NOT NULL REFERENCES pilgrims(id) ON DELETE CASCADE, checkpoint_id TEXT NOT NULL REFERENCES attendance_checkpoints(id) ON DELETE RESTRICT, device_latitude TEXT NOT NULL, device_longitude TEXT NOT NULL, accuracy_m DOUBLE PRECISION NOT NULL, distance_m DOUBLE PRECISION NOT NULL, checked_by TEXT NOT NULL, checked_at TEXT NOT NULL);
    ...
    `);
    await client.query(`ALTER TABLE pilgrims ADD COLUMN IF NOT EXISTS baptismal_name TEXT NOT NULL DEFAULT ''; ALTER TABLE pilgrims ADD COLUMN IF NOT EXISTS email TEXT NOT NULL DEFAULT ''; ALTER TABLE pilgrims ADD COLUMN IF NOT EXISTS preferred_language TEXT NOT NULL DEFAULT 'en'; ALTER TABLE pilgrims ADD COLUMN IF NOT EXISTS access_token TEXT; ALTER TABLE pilgrims ADD COLUMN IF NOT EXISTS access_token_hash TEXT; ALTER TABLE pilgrims ADD COLUMN IF NOT EXISTS access_token_expires_at TEXT; CREATE UNIQUE INDEX IF NOT EXISTS idx_pilgrims_access_token_hash ON pilgrims(access_token_hash);`);
...
function setupSqlite() {
  ...
  try {
    ...
    db.exec(`
      ...
      CREATE TABLE IF NOT EXISTS pilgrim_meal_logs (id TEXT PRIMARY KEY, pilgrim_id TEXT NOT NULL REFERENCES pilgrims(id) ON DELETE CASCADE, meal_type TEXT NOT NULL, note TEXT NOT NULL DEFAULT '', recorded_by TEXT NOT NULL, recorded_at TEXT NOT NULL);
      CREATE TABLE IF NOT EXISTS attendance_checkpoints (id TEXT PRIMARY KEY, name TEXT NOT NULL, postcode TEXT NOT NULL DEFAULT '', address TEXT NOT NULL, address_detail TEXT NOT NULL DEFAULT '', latitude REAL NOT NULL, longitude REAL NOT NULL, radius_m INTEGER NOT NULL DEFAULT 100, active INTEGER NOT NULL DEFAULT 1, created_by TEXT NOT NULL, created_at TEXT NOT NULL, updated_at TEXT NOT NULL);
      CREATE TABLE IF NOT EXISTS pilgrim_attendance_logs (id TEXT PRIMARY KEY, pilgrim_id TEXT NOT NULL REFERENCES pilgrims(id) ON DELETE CASCADE, checkpoint_id TEXT NOT NULL REFERENCES attendance_checkpoints(id) ON DELETE RESTRICT, device_latitude TEXT NOT NULL, device_longitude TEXT NOT NULL, accuracy_m REAL NOT NULL, distance_m REAL NOT NULL, checked_by TEXT NOT NULL, checked_at TEXT NOT NULL);
      ...
      CREATE INDEX IF NOT EXISTS idx_qna_status ON qna_posts(status, created_at);
    `);
    const pilgrimColumns = db.prepare("PRAGMA table_info(pilgrims)").all();
    const pilgrimMigrationColumns = [["baptismal_name", "TEXT NOT NULL DEFAULT ''"], ["email", "TEXT NOT NULL DEFAULT ''"], ["preferred_language", "TEXT NOT NULL DEFAULT 'en'"], ["access_token", "TEXT"], ["access_token_hash", "TEXT"], ["access_token_expires_at", "TEXT"]];
    for (const [name, definition] of pilgrimMigrationColumns) {
      if (!pilgrimColumns.some((column) => column.name === name)) db.exec(`ALTER TABLE pilgrims ADD COLUMN ${name} ${definition}`);
    }
    db.exec("CREATE UNIQUE INDEX IF NOT EXISTS idx_pilgrims_access_token_hash ON pilgrims(access_token_hash)");
